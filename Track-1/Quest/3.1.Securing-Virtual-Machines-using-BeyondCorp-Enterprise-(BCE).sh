...
gcloud compute instances create linux-iap --zone=us-east1-b --machine-type=n1-standard-1 --network-interface=subnet=default,no-address --metadata=enable-oslogin=true

gcloud compute instances create windows-iap --zone=us-east1-b --machine-type=n1-standard-1 --network-interface=subnet=default,no-address --metadata=enable-oslogin=true --maintenance-policy=MIGRATE --provisioning-model=STANDARD --create-disk=auto-delete=yes,boot=yes,device-name=windows-iap,image=projects/windows-cloud/global/images/windows-server-2016-dc-v20220624,mode=rw,size=50,type=projects/$PROJECT_ID/zones/us-east1-a/diskTypes/pd-balanced --no-shielded-secure-boot --shielded-vtpm --shielded-integrity-monitoring --reservation-affinity=any

gcloud compute instances create windows-connectivity --zone=us-east1-b --machine-type=n1-standard-1 --network-interface=network-tier=PREMIUM,subnet=default --metadata=enable-oslogin=true --maintenance-policy=MIGRATE --provisioning-model=STANDARD --scopes=https://www.googleapis.com/auth/cloud-platform --create-disk=auto-delete=yes,boot=yes,device-name=windows-connectivity,image=projects/windows-cloud/global/images/windows-server-2016-dc-v20220624,mode=rw,size=50,type=projects/$PROJECT_ID/zones/us-east1-a/diskTypes/pd-balanced --no-shielded-secure-boot --shielded-vtpm --shielded-integrity-monitoring --reservation-affinity=any
...
read -p "${BOLD}${YELLOW}Completed both Task? [ y/n ] : ${RESET}" PROCEED

while [ $PROCEED != 'y' ];
do echo " Do it manually " && 
sleep 10 && 
read -p "${BOLD}${YELLOW}Completed both Task? [ y/n ] : ${RESET}" PROCEED ;
done
...
sleep 20
gcloud compute reset-windows-password windows-connectivity --zone us-east1-a --quiet --user=admin > windows-connectivity.rdp
...
Run this in ssh
${BG_RED}
gcloud compute start-iap-tunnel windows-iap 3389 --local-host-port=localhost:0  --zone=us-east1-a
exit
${RESET}"

rm *
gcloud compute ssh linux-iap --zone us-east1-a --quiet
completed "Task 4"
